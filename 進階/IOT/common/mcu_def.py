...
class mcu_fun:
    ...
    def led_initial(self, r_pin, g_pin, b_pin, mode: str = 'digital'):
        if mode.lower() == 'digital':
            ...
            RED = PWM(Pin(r_pin), freq=frequency, duty=duty_cycle(50))
            GREEN = PWM(Pin(g_pin), freq=frequency, duty=duty_cycle(50))
            BLUE = PWM(Pin(b_pin), freq=frequency, duty=duty_cycle(50))
